...
**Cost**: 1 credit per result returned. If `max_results: 25` and the query matches 25+ companies, the request costs 25 credits.
...
<Steps>
  ...
  <Step title="Find decision-makers">
    Pass each `linkedin_url` to the [Waterfall ICP Search](/guide/concepts/waterfall-logic) to find the right contacts, or use [People Search](/guide/concepts/people-search) to find multiple contacts across all your target companies in a single query.
  </Step>
