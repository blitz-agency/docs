...
The **Waterfall ICP** engine (`/api/search/waterfall-icp`) is designed to solve one specific problem: **Finding the single best decision-maker** at a target company without manual sorting.
...
```json
{
  "company_linkedin_url": "[https://www.linkedin.com/company/wttj-fr](https://www.linkedin.com/company/wttj-fr)",
  "cascade": [
    {
...
<Tabs>
  ...
  <Tab title="Real-Time (Live)">
    **Endpoint**: `/search/waterfall-icp-real-time`

    Best for **niche targeting** or when Standard returns no results. It queries the live network at the moment of the request.

    - **Cost**: 3 Credits / result
    - **Latency**: 3-6 seconds
    - **Ideal for**: Finding specific roles in small SMBs or verifying job changes.
  </Tab>
</Tabs>
