...
<Note>
  **Paid plans**: All endpoints are **unlimited** with no per-request cost. Credits below apply only to **free trial accounts** (1,000 credits). Email enrichment requires Unlimited Email plan+; phone enrichment requires Unlimited Phone Numbers plan.
</Note>

| Endpoint | Method | Description | Plan | Trial cost |
| :------- | :----- | :---------- | :--- | :--------- |
| `/v2/account/key-info` | `GET` | Check API key validity, credits, and rate limit | Any | 0 credits |
| `/v2/search/waterfall-icp-keyword` | `POST` | Find decision-makers via cascade hierarchy (cached data, <600ms) | Leads+ | 1 / result |
| `/v2/search/employee-finder` | `POST` | Search employees by role and seniority level | Leads+ | 1 / result |
| `/v2/search/companies` | `POST` | Find companies by filters (industry, size, location, keywords) | Leads+ | 1 / result |
| `/v2/search/people` | `POST` | Find people across companies by combined company + people filters | Leads+ | 1 / result |
| `/v2/enrichment/email` | `POST` | LinkedIn profile URL → verified work email | Email+ | 1 (0 if not found) |
| `/v2/enrichment/phone` | `POST` | LinkedIn profile URL → phone number (**US only**) | Phone | 5 (0 if not found) |
| `/v2/enrichment/email-to-person` | `POST` | Work email → full person profile | Leads+ | 1 |
| `/v2/enrichment/phone-to-person` | `POST` | Phone number → full person profile | Leads+ | 5 |
| `/v2/enrichment/company` | `POST` | Company LinkedIn URL → full company profile | Leads+ | 1 |
| `/v2/enrichment/domain-to-linkedin` | `POST` | Website domain → Company LinkedIn URL | Leads+ | 1 |
| `/v2/enrichment/linkedin-to-domain` | `POST` | Company LinkedIn URL → verified email domain | Leads+ | 0.5 |
| `/v2/utilities/email/validate` | `POST` | Validate email deliverability via SMTP handshake | Leads+ | 0.5 |
| `/v2/utilities/current-date` | `POST` | Get current server date/time for a timezone | Any | 0 |
...
| `402` | Payment Required | Insufficient credits. Top up or upgrade your plan. |
